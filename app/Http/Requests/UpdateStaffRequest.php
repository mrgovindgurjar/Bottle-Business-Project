...
{
    public function authorize(): bool { return auth()->user()?->hasPermission('staff.edit') === true; }
    public function rules(): array { $staff = $this->route('staff'); $id = $staff instanceof Staff ? $staff->id : $staff; return [
        'employee_code'=>['nullable','string','max:50',Rule::unique('staff','employee_code')->ignore($id)],
        'first_name'=>['required','string','max:100'], 'last_name'=>['nullable','string','max:100'],
        'email'=>['nullable','email','max:150',Rule::unique('staff','email')->ignore($id)], 'mobile'=>['nullable','string','max:30'],
        ...
        'salary'=>['nullable','numeric','min:0'], 'address'=>['nullable','string','max:500'], 'city'=>['nullable','string','max:100'],
        'state'=>['nullable','string','max:100'], 'pincode'=>['nullable','string','max:20'],
